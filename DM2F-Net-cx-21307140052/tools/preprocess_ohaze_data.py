...
if __name__ == '__main__':
    ...
    for idx, img_name in enumerate(tqdm(train_list)):
        ...
        # mean_value = np.mean(dark_channel)
        # judge = dark_channel < mean_value
        # judge = judge * -0.5 + 1
        # dark_image = np.zeros(img_np.shape)
        # dark_image[:, :, 0] = img_np[:, :, 0] * judge
        # dark_image[:, :, 1] = img_np[:, :, 1] * judge
        # dark_image[:, :, 2] = img_np[:, :, 2] * judge
        dark_image = img_np * 0.4
        dark_image = Image.fromarray(dark_image.astype(np.uint8))
        #dark gt
        # dark_channel = np.min(gt_np, axis=2)
        # mean_value = np.mean(dark_channel)
        # judge = dark_channel < mean_value
        # judge = judge * -0.5 + 1
        # dark_gt = np.zeros(gt_np.shape)
        # dark_gt[:, :, 0] = gt_np[:, :, 0] * judge
        # dark_gt[:, :, 1] = gt_np[:, :, 1] * judge
        # dark_gt[:, :, 2] = gt_np[:, :, 2] * judge
        dark_gt = gt_np * 0.4
        dark_gt = Image.fromarray(dark_gt.astype(np.uint8))
        ...
        for h_idx in range(h_steps):
            for w_idx in range(w_steps):
                ...
                #
                dark_image_crop = dark_image.crop((ws0, hs0, ws1, hs1))
                dark_gt_crop = dark_gt.crop((ws0, hs0, ws1, hs1))
                ...
                dark_image_crop.save(os.path.join(patch_haze_path, '{}_h_{}_w_{}_dark.png'.format(img_f_name, h_idx, w_idx)))
                dark_gt_crop.save(os.path.join(patch_gt_path, '{}_h_{}_w_{}_dark.png'.format(gt_f_name, h_idx, w_idx)))
